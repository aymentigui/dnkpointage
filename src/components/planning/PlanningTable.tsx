                        ...
                        {history.map((h, idx) => (
                            <div key={idx} className="space-y-0.5">
                                ...
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <StatutBadge code={h.nouveau_statut} />
                                ...
                                <div className="flex items-center justify-between text-[9px] text-slate-500 pl-0.5">
                                    {h.modifie_par ? (
                                        ...
                                        <span className="italic">Inconnu</span>
                                    )}
                                    <span className="shrink-0 ml-2">
                                        {new Date(h.modifie_le).toLocaleDateString('fr-FR', {
                                            day: '2-digit', month: '2-digit',
                                        })}{' '}
                                        {new Date(h.modifie_le).toLocaleTimeString('fr-FR', {
...
interface AnnotModalProps {
    ...
    onOptimisticUpdate: (updates: {
        matricule: string; date: string; statut: string
...
function AnnotModal({
    ...
    const handleSave = async () => {
        if (!canSave) return
        setSaving(true)
        const updates = cells.map(({ matricule, date }) => ({
            matricule, date,
            statut: annot ? 'A' : base,
            annotCode: annot || undefined,
            annotLibelle: annot ? getAnnotLabel(annot) : undefined,
        }))
        onOptimisticUpdate(updates)
        onOpenChange(false)
        toast.success(cells.length > 1 ? `${cells.length} cellules modifiées` : 'Enregistré')
        try {
            await Promise.all(cells.map(({ employeeId, date }) =>
                annot
                    ? planningApi.update(employeeId, {
                        date, statut: 'A',
                        annotation: { code: annot, libelle: getAnnotLabel(annot), description: desc || undefined },
                    })
    ...
    const label = cells.length === 1
        ? `${cells[0].matricule} · ${new Date(cells[0].date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}`
        : `${cells.length} cellules sélectionnées`
            ...
            <DialogContent className="sm:max-w-[460px] p-0 overflow-hidden gap-0">
                ...
                <div className="px-6 py-4 flex items-center justify-between border-t border-slate-100">
                    <span className="text-xs text-slate-400">
                        {annot
                            ? <><span className="font-mono font-bold text-slate-600">{annot}</span>{cells.length > 1 && ` · ${cells.length} cellules`}</>
                            : base
                                ? <span className="font-medium text-slate-600">{ANNOT_BASE.find(t => t.code === base)?.label}</span>
                                : 'Aucune sélection'}
                    </span>
                    <div className="flex gap-2">
                        ...
                        <Button size="sm" onClick={handleSave} disabled={saving || !canSave} className="min-w-[90px]">
                            {saving ? '...' : `Enregistrer${cells.length > 1 ? ` (${cells.length})` : ''}`}
                        </Button>
                    </div>
                </div>
            </DialogContent>
...
const EMPTY_STATS: EmpStats = {
    ...
    absences_annotees: 0, repos: 0, presences_supplementaires: 0, total: 0,
}
...
export function PlanningTable({
    ...
    // ── Tooltip ───────────────────────────────────────────────
    const TOOLTIP_EMPTY: TooltipState = {
        visible: false, history: [], statut: '',
        statut_original: null, date: '', posX: 0, posY: 0,
    }
    ...
    const handleOptimisticUpdate = useCallback((
        ...
        updates.forEach(({ matricule, date, statut, annotCode, annotLibelle }) => {
            ...
            const pIdx = plannings.findIndex(
                (p: any) => (p.date?.split?.('T')?.[0] ?? p.date) === date
            )
            ...
            const aIdx = annotations.findIndex(
                (a: any) => (a.date?.split?.('T')?.[0] ?? a.date) === date
            )
            if (annotCode) {
    ...
    const getDisplayCode = (matricule: string, date: string): string => {
        ...
        if (!empData) return ''
        const annot = empData.annotations?.find(
            (a: any) => (a.date?.split?.('T')?.[0] ?? a.date) === date
        )
        if (annot) return annot.code
        return empData.plannings?.find(
            (p: any) => (p.date?.split?.('T')?.[0] ?? p.date) === date
        )?.statut || ''
    }
    ...
    const getCellMeta = (matricule: string, date: string): {
        ...
        const plan = empData.plannings?.find(
            (p: any) => (p.date?.split?.('T')?.[0] ?? p.date) === date
        )
        return {
            history: plan?.history ?? [],
            statut_original: plan?.statut_original ?? null,
        }
    }

    const getStats = (matricule: string): EmpStats =>
        planningData[matricule]?.stats ?? EMPTY_STATS
                                ...
                                <TableRow key={emp.id}
                                    ...
                                    <TableCell className={cn('sticky left-0 z-10 p-0 w-[200px]', isEmpSel ? 'bg-slate-900/[0.04]' : 'bg-white')}>
                                        <div className="flex items-stretch h-full min-h-[36px]">
                                            ...
                                            <button
                                                onClick={e => { e.stopPropagation(); router.push(`/workspaces/${workspaceId}/employees/${emp.id}`) }}
                                                className="w-7 flex items-center justify-center shrink-0 text-slate-300 hover:text-slate-600 hover:bg-slate-100 transition-colors border-l border-slate-100"
                                    ...
                                    {dates.map(date => {
                                        ...
                                        return (
                                            <TableCell
                                                key={cellKey}
                                                onClick={e => handleCellClick(emp.matricule, date, e)}
                                                onMouseEnter={e => showTooltip(e, history, code, statut_original, date)}
                                                onMouseLeave={hideTooltip}
                                                className={cn(
                                                    'text-center p-0 cursor-pointer transition-all min-w-[36px] h-8 select-none relative',
                                                    isWE && !colors && 'bg-slate-50/60',
                                                    isCellSel && 'ring-2 ring-inset ring-slate-700 z-10'
                                                )}
                ...
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-slate-900 text-white px-4 py-2.5 rounded-2xl shadow-2xl animate-in slide-in-from-bottom-3 duration-200">
                    <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-white/15 flex items-center justify-center">
                            <span className="text-[11px] font-bold">{nbCellsSelected}</span>
                        </div>
                        <span className="text-sm text-slate-300">
                            {nbCellsSelected === 1 ? 'cellule sélectionnée' : 'cellules sélectionnées'}
                        </span>
                    </div>
                    <div className="w-px h-5 bg-white/20" />
                    <button onClick={openAnnotModalForSelected}
            ...
            {selectedEmployees.size > 0 && nbCellsSelected === 0 && (
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-indigo-950 text-white px-4 py-2.5 rounded-2xl shadow-2xl animate-in slide-in-from-bottom-3 duration-200">
                    ...
                    <button onClick={() => setCycleModalOpen(true)}
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-white text-indigo-900 rounded-xl text-xs font-semibold hover:bg-indigo-50 transition-colors">
                        <RefreshCw className="w-3.5 h-3.5" />Modifier le cycle
                    </button>
                    <button onClick={clearEmployeeSelection}
                        className="p-1.5 rounded-lg hover:bg-white/10 transition-colors text-indigo-400 hover:text-white">
                        <X className="w-3.5 h-3.5" />
                    </button>
                </div>
            )}
...
function recalculerStats(empData: any): EmpStats {
    let presents = 0, absences = 0, absences_annotees = 0, repos = 0

    empData.plannings?.forEach((p: any) => {
        const dateStr = p.date?.split?.('T')?.[0] ?? p.date
        const hasAnnot = empData.annotations?.some(
            (a: any) => (a.date?.split?.('T')?.[0] ?? a.date) === dateStr
        )
