...
const RegisterSchema = (u: any) =>
  z
    .object({
      ...
      email: z
        .string({ required_error: u("emailrequired") })
        .email({ message: u("emailinvalid") }),
      password: z
        .string({ required_error: u("passwordrequired") })
        .min(6, { message: u("password6") }),
      passwordConfirm: z
        .string({ required_error: u("confirmpasswordrequired") })
        .min(6, { message: u("password6") }),
    })
...
export async function registerUser(data: any): Promise<AuthResponse> {
  ...
  try {
    ...
    if (existingUserByUsername) {
      return {
        status: 400,
        data: { message: u("usernameexists") || "Username already exists" },
      };
    }
    ...
    if (existingUserByEmail) {
      return {
        status: 400,
        data: { message: u("emailexists") || "Email already exists" },
      };
    }
...
export async function loginUser(data: any): Promise<AuthResponse> {
  ...
  try {
    ...
    if (!user || !user.password) {
      return {
        status: 401,
        data: { message: u("invalidcredentials") || "Invalid credentials" },
      };
    }
    ...
    if (lockCheck.isLocked) {
      return {
        status: 423,
        data: { message: lockCheck.message || u("accountlocked") },
      };
    }
    ...
    if (!isValid) {
      ...
      if (lockedUntil) {
        errorMessage =
          u("accountlocked") ||
          "Your account has been locked for 15 minutes due to too many failed attempts.";
    ...
    if (user.email_verified === null) {
      return {
        status: 403,
        data: {
          message: u("emailnotverified") || "You must verify your email",
    ...
    if (user.is_two_factor_enabled && user.email) {
      ...
      if (twoFALockCheck.isLocked) {
        return {
          status: 429,
          data: { message: twoFALockCheck.message || u("twofactorblocked") },
        };
      }
      ...
      if (!code) {
        ...
        await send2FACode({
          email: user.email,
          code: tokenResponse.data.token,
        });
        return {
          status: 202,
          data: { twoFactorConfermation: true, message: s("2fa_code_sent") },
        };
      } else {
        ...
        if (
          ...
          new Date(token.data.expiredAt) < new Date()
        ) {
          ...
          let errorMessage =
            e("invalideorexpiredcode") || "Invalid or expired code";

          if (twoFactorBlockedUntil) {
            errorMessage =
              u("twofactorblocked") ||
              "2FA verification blocked for 15 minutes due to too many failed attempts.";
          } else if (remainingAttempts > 0) {
 ...
 */
export async function confermationRegister(
  data: any,
  email: string,
  ...
  try {
    ...
    if (
      ...
      new Date(user.verification_blocked_until) > new Date()
    ) {
    ...
    await prisma.user.update({
      ...
      data: {
        ...
        verification_blocked_until:
          user.verification_attempts + 1 >= 3 ? fifteenMinutesFromNow : null,
      },
    });
...
async function checkAndHandleAccountLock(
  user: any,
...
async function checkAndHandle2FALock(
  user: any,
): Promise<{ isLocked: boolean; message?: string }> {
  const u = await getTranslations("Users");
  const s = await getTranslations("System");
  ...
  if (
    user.two_factor_blocked_until &&
    new Date(user.two_factor_blocked_until) > new Date()
  ) {
    const remainingTime = Math.ceil(
      (new Date(user.two_factor_blocked_until).getTime() - Date.now()) /
        (1000 * 60),
    );
    return {
      isLocked: true,
...
export async function canManageEmployee(userId: string, employeeId: string) {
  const employee = await prisma.employee.findUnique({
    where: { id: employeeId },
    include: {
      userManagedEmployees: { where: { user_id: userId } },
      departmenet: {
        include: { userManagedDepartmenets: { where: { user_id: userId } } },
      },
    },
  });
  ...
  // S'il est géré directement
  if (employee.userManagedEmployees.length > 0) return true;
  // S'il n'a pas de département, il n'est géré par personne
  if (!employee.departmenet) return false;
  // S'il est géré via son département
  if (employee.departmenet?.userManagedDepartmenets.length > 0) return true;
