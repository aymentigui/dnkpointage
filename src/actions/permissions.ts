...
export async function haveSession() {
  ...
  if (session.status !== 200 || !session.data || !session.data.user) {
    ...
    if (LogoutUser.status === 200) return redirect("/auth/login");
  }
...
export async function getUserRolesNames(
  id: string,
): Promise<{ status: number; data: any }> {
  const e = await getTranslations("Error");
  try {
...
export async function getUserRolesId(
  id: string,
): Promise<{ status: number; data: any }> {
  const e = await getTranslations("Error");
  try {
...
export async function ISADMIN(
  ...
  try {
    ...
    return {
      status: 200,
      data: { is_admin: is_admin?.is_admin ? true : false },
    };
  } catch (error) {
